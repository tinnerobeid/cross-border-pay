...
export interface RegisterPayload { full_name: string; email: string; password: string; phone: string }
...
export async function getTransfers(token: string): Promise<TransferOut[]> {
  return handleResponse(await authFetch(BASE_URL + '/transfers', { method: 'GET' }, token));
}
