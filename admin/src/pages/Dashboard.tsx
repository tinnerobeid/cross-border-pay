  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        />
        <StatCard title="Volume Today" value={`TZS ${fmt(stats?.volume_today ?? 0)}`}
          subtitle={`Total: TZS ${fmt(stats?.volume_total ?? 0)}`} loading={loading} color="green"
        ...
        />
        <StatCard title="Fees Earned" value={`$${(stats?.total_fees_earned ?? 0).toFixed(2)}`}
          subtitle={`$${(stats?.fees_earned_today ?? 0).toFixed(2)} today`} loading={loading} color="green"
